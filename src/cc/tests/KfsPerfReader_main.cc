//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: KfsPerfReader_main.cc 3345 2011-11-17 21:33:24Z sriramr $
//
// Created 2006/06/23
//
...
static off_t doRead(const string &kfspathname,
    int numMBytes, size_t readSizeBytes, int cliBufSize, int readAhead, double sleepSec);
...
doRead(const string &filename, int numMBytes,
    size_t readSizeBytes, int cliBufSize, int readAhead, double sleepSec)
{
    const int mByte = 1024 * 1024;
    boost::scoped_array<char> dataBuf;
    size_t bytesRead = 0;
    int res, nMBytes = 0, fd;
    off_t nread = 0;
