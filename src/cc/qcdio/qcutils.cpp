//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: qcutils.cpp 3298 2011-11-11 21:49:20Z sriramr $
//
// Created 2008/11/01
//
    ...
    int64_t inSize)
{
    ...
    int fallocStatus = posix_fallocate(inFd, 0, inSize);
    if (fallocStatus < 0) {
            return (errno > 0 ? -errno : (errno ? errno : -1));
    }
    return inSize;
#endif /* KFS_USE_FALLOCATE */

    return (inFd < 0 ? -EINVAL : 0);
}

    /* static */ int64_t
QCUtils::UnReserveFileSpace(
    int     inFd,
    int64_t inOffset,
    int64_t inLen)
{
    ...
    if (platform_test_xfs_fd(inFd)) {
        xfs_flock64_t theResv = {0}; // Clear to make valgrind happy.
        theResv.l_whence = 0;
        theResv.l_start  = inOffset;
        theResv.l_len    = inLen;
        if (xfsctl(0, inFd, XFS_IOC_UNRESVSP64, &theResv)) {
            return (errno > 0 ? -errno : (errno ? errno : -1));
        }
        return inLen;
    }
