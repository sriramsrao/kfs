//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: BufferedLogWriter.cc 2129 2011-03-08 21:02:59Z sriramr $
//
// Created 2010/03/02
//
...
public:
    ...
    {
        QCStMutexLocker theLocker(mMutex);
        AppendSelf(inLogLevel, -1, inFmtStrPtr, inArgs);
    }
    void AppendSelf(
        LogLevel    inLogLevel,
        const char* inStrPtr,
        int         inStrLen)
    {
        va_list theArgs; // dummy
        AppendSelf(inLogLevel, max(0, inStrLen), inStrPtr, theArgs);
    }
    void AppendSelf(
        LogLevel    inLogLevel,
        int         inStrLen,
